...
```shell
os6905@Huanan1:~/lab6$ vim test.cpp
os6905@Huanan1:~/lab6$ g++ test.cpp -o test
os6905@Huanan1:~/lab6$ ./test

选择调度算法：
1: FIFO 2: 时间片轮换 3: 优先级调度 4: 最短时间优先 
1
输入进程个数:
3
输入此进程时间片大小: 
10
输入第1个进程的名字、cpu时间和优先级:
P1 10 1

输入第2个进程的名字、cpu时间和优先级:
P2 10 2

输入第3个进程的名字、cpu时间和优先级:
P3 10 3

(说明: 在本程序所列进程信息中，优先级一项是指进程运行后的优先级!!)

进程名字共需占用CPU时间 还需要占用时间 优先级进程P1正在运行...
...
所有进程都已经执行完毕!
os6905@Huanan1:~/lab6$ cat test.cpp
#include <iostream>
#include <string>
using namespace std;
