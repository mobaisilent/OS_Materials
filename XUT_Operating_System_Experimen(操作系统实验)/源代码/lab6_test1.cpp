...
void sort1(Process pr[], int size); // 此排序后按需要的cpu 时间从小到大排列
...
void FIFO(Process process[], int num, int Timepice) {
    // process[] 是输入的进程，num 是进程的数目，Timepice 是时间片大小
    while (true) {
        if (num == 0) {
        ...
        if (process[0].LeftTime == 0) {
            ...
            cout << "进程" << process[0].ProcessName << "正在运行..." << endl; // 输出正在运行的进程信息
            process[0].LeftTime -= Timepice; // 模拟进程运行
            if (process[0].LeftTime < 0) process[0].LeftTime = 0; // 避免剩余时间为负
        }
        cout << endl;
        cin.get();
        cout << endl;
...
int main() {
    ...
    cout << endl; cout << endl;
    cout << "进程名字" << "共需占用CPU时间 " << "还需要占用时间 " << "优先级";
    if(a==1)
